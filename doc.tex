...
Eksperymenty przeprowadzono w trybie wsadowym (bez warstwy wizualizacji), uruchamiając model wielokrotnie dla różnych zestawów parametrów. Dla każdej konfiguracji wykonano niezależne replikacje (z uwagi na losowość rozmieszczenia punktów startowych, przydziału miejsc oraz losowej kolejności przeglądania sąsiedztwa w algorytmie ruchu), a prezentowane wartości stanowią uśrednienie po replikacjach. Wąsy na wykresach oznaczają odchylenie standardowe.

Jako główne miary efektywności przyjęto:
\begin{itemize}
    ...
    \item \textbf{Średni czas wejścia} --- średnią liczbę kroków, po której pojedynczy kibic zajmuje swoje miejsce (miara komfortu/sprawności z perspektywy widza);
    \item \textbf{Średnią długość kolejki} --- średnią w czasie liczbę kibiców oczekujących na odprawę (miara natężenia wąskiego gardła).
\end{itemize}

O ile nie zaznaczono inaczej, konfiguracją bazową było: 200 kibiców, 4 bramki, szerokość bramki 2 komórki, czas kontroli biletu 5 kroków oraz świadomy wybór bramki. W kolejnych eksperymentach zmieniano jeden wybrany parametr, pozostawiając resztę bez zmian.
